...
def resolve_task_type(req: TaskRequest, input_text: str) -> str:
    """Resolve task type — use explicit selection or auto-detect via router."""
    if req.task_type and req.task_type != "auto":
        return req.task_type
    result = classify_task(input_text)
    if result is None:
        return "financing_deck"  # fallback
    return result
...
def run_task_async(task_id: str, req: TaskRequest):
    """
    Execute the full FCC workflow chain in a background thread.
    Uses the real V1 engine: build_system_prompt → build_user_message → call_department.
    """
    task = tasks[task_id]
    try:
        ...
        task["chain"] = [
            {
                "department": config.DEPARTMENTS[dept_id]["name"],
                "dept_id": dept_id,
                "status": "pending",
            }
            for dept_id in chain
        ]
        # Mark last command_center step as Final Integration
        ...
        for i, dept_id in enumerate(chain):
            ...
            try:
                ...
                # Call the real Anthropic API via client.py
                output_text = call_department(
                ...
                logger.info(
                    "[Task %s] Step %d/%d: %s completed (%.1fs)",
                    task_id, i + 1, total_steps, dept["name"], duration,
                )
        ...
        logger.info(
            "[Task %s] Workflow '%s' completed in %.1fs",
            task_id, workflow_name, task["total_duration"],
        )
...
def get_task_status(task_id: str):
    """Get current status of a task including step progress."""
    if task_id not in tasks:
        raise HTTPException(status_code=404, detail="Task not found")

    task = tasks[task_id]
    current_step = -1
    for i, s in enumerate(task["chain"]):
        if s["status"] == "running":
            current_step = i
            break
    if current_step == -1 and task["status"] == "completed":
        current_step = len(task["chain"]) - 1
...
def get_task_trace(task_id: str):
    """Get detailed trace of task execution."""
    if task_id not in tasks:
        raise HTTPException(status_code=404, detail="Task not found")
        ...
        "tasks": [
            {
                ...
                "workflow_key": t.get("workflow_key", ""),
...
def get_config():
    ...
    return {
        "departments": {
            ...
            for k, v in config.DEPARTMENTS.items()
        },
        "workflows": {
            k: {"name": v["name"], "description": v["description"], "chain": v["chain"]}
